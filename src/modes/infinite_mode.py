...
class InfiniteMode(BaseMode):
    ...
    def draw_hud(self):
        ...
        hud_surface = pygame.Surface((self.WIDTH, 100)) # 
        hud_surface.fill((0, 0, 0)) # 
        hud_surface.set_alpha(50) # 
        ...
        record_text = f"Record: {self.best_time:.1f}s"
        text = font.render(record_text, True, (255, 215, 0))
        ...
        text = font.render(vel_text, True, vel_color)
    ...
    def show_game_over(self, final_time):
        ...
        while showing_end:
            ...
            # Game Over con efecto parpadeante
            text_alpha = abs(math.sin(time.time() * 3)) * 255
            game_over = font_big.render("PERDISTE", True, (255, 0, 0))
            ...
            # Tiempo final
            score = font_big.render(f"{final_time:.1f}s", True, (255, 255, 255))
            score_rect = score.get_rect(center=(self.WIDTH//2, self.HEIGHT//2 + 20))
            self.screen.blit(score, score_rect)
            
            # Instrucciones principales
            instructions = font_small.render("R - Reintentar    ESC - Menú", True, (200, 200, 200))
            inst_rect = instructions.get_rect(center=(self.WIDTH//2, self.HEIGHT - 50))
            self.screen.blit(instructions, inst_rect)
            
            # Texto para resetear récord (semi-transparente)
            reset_text = font_small.render("Presiona F para resetear récord", True, (150, 150, 150))
